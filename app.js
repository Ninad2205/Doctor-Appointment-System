...
app.post("/login", (req, res) => {
    ...
    let sql = userType === "doctor" ? "SELECT * FROM doctors WHERE username = ?" : "SELECT * FROM patients WHERE username = ?";
    ...
    db.query(sql, [username], async (err, results) => {
        ...
        res.redirect(userType === "doctor" ? "/doctor-dashboard" : "/patient-dashboard");
    });
});
...
app.post("/accept-appointment/:id", verifyToken, (req, res) => {
    ...
    const sql = "UPDATE appointments SET status = 'Accepted' WHERE id = ? AND doctor_id = ?";
...
app.get("/edit-doctor-profile", verifyToken, (req, res) => {
    ...
    const doctorQuery = "SELECT doctor_id, name, contact_no, specialization, gender FROM doctors WHERE doctor_id = ?";
...
app.post("/update-doctor-profile", verifyToken, (req, res) => {
    ...
    db.query(sql, [name, contact_no, gender, specialization, doctor_id], (err, result) => {
        if (err) {
...
app.get("/edit-profile", verifyToken, (req, res) => {
    ...
    db.query("SELECT * FROM patients WHERE patient_id = ?", [req.user.id], (err, results) => {
        if (err || results.length === 0) {
...
app.post("/edit-profile", verifyToken, async (req, res) => {
    ...
    try {
        let sql = "UPDATE patients SET name = ?, contact_no = ?, gender = ? WHERE patient_id = ?";
        let values = [name, contact_no, gender, userId];

        // If password is provided, hash it before updating
        if (password && password.trim() !== "") {
            const hashedPassword = await bcrypt.hash(password, 10);
            sql = "UPDATE patients SET name = ?, contact_no = ?, gender = ?, password = ? WHERE patient_id = ?";
...
app.get("/delete-account", verifyToken, (req, res) => {
    ...
    db.query("DELETE FROM patients WHERE patient_id = ?", [req.user.id], (err) => {
...
app.post("/register-patient", async (req, res) => {
    ...
    const sql = "INSERT INTO patients (patient_id, name, username, password, contact_no, gender) VALUES (?, ?, ?, ?, ?, ?)";
    
    db.query(sql, [patient_id, name, username, hashedPassword, contact_no, gender], (err, result) => {
        if (err) {
            console.error(err);
...
app.post("/register-doctor", async (req, res) => {
    const { doctor_id, name, username, password, contact_no, specialization, gender } = req.body;

    // Hash the password securely
    const hashedPassword = await bcrypt.hash(password, 10);

    // Corrected SQL query (including specialization)
    const sql = "INSERT INTO doctors (doctor_id, name, username, password, contact_no, specialization, gender) VALUES (?, ?, ?, ?, ?, ?, ?)";

    db.query(sql, [doctor_id, name, username, hashedPassword, contact_no, specialization, gender], (err, result) => {
        if (err) {
...
app.post("/update-appointment/:id", verifyToken, async (req, res) => {
    try {
        ...
        const formattedTime = selectedDate.toISOString().slice(0, 19).replace("T", " ");

        const sql = "UPDATE appointments SET contact_no = ?, symptoms = ?, appointment_time = ? WHERE id = ? AND patient_id = ?";
        const [result] = await db.promise().execute(sql, [contact_no, symptoms, formattedTime, appointmentId, req.user.id]);
...
router.get('/edit-appointment/:id', async (req, res) => {
    ...
    try {
        ...
        appointment.appointment_time = new Date(appointment.appointment_time).toISOString().slice(0, 16);
...
app.post("/update-appointment/:id", verifyToken, (req, res) => {
    ...
    db.query("UPDATE appointments SET contact_no = ?, symptoms = ?, appointment_time = ? WHERE id = ? AND patient_id = ?", 
...
app.post("/submit-feedback", verifyToken, (req, res) => {
    ...
    db.query("SELECT doctor_id FROM doctors WHERE doctor_id = ?", [doctor_id], (err, doctorResults) => {
        if (err || doctorResults.length === 0) {
...
app.get('/feedback/:doctor_id', verifyToken, async (req, res) => {
    ...
    try {
        const [doctor] = await db.execute("SELECT doctor_id, name FROM doctors WHERE doctor_id = ?", [doctor_id]);
